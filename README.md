...
## Android 3.0: кандидат обновления

Новая очередь, сохранение истории и обновлённый интерфейс подготовлены отдельно от рабочего сервера.
Порядок проверки, сборки и ограничения выпуска: [Android 3.0 review](docs/android-3.0-review.md).
Тестовый APK устанавливается отдельно. Для обновления рабочего приложения без потери данных нужен исходный ключ подписи; удалять приложение перед установкой нельзя.
